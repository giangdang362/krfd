...
const DataFundingVoteTable: FC<DataFundingVoteTableProps> = ({
  ...
  const handleGetFundingVote = async (isFilterChange?: boolean) => {
    setLoading(true);
    if (isFilterChange) {
  ...
  useEffect(() => {
    handleGetFundingVote(true);
    handleGetCurFundingVote();
  }, [curFundingVote, currentStatus, reload, currentIdol, currentName, reloadAllVote]);
    ...
    <div className="wrapp-table">
      ...
      >
        ...
        >
          ...
          {curDrawerFundingVote?.status === STATUS_TYPE.BOOKING_TYPE && (
            <div style={{ display: 'flex' }}>
              <div style={{ width: '108px', fontSize: '14px', fontWeight: 400, color: '#616161' }}>
                {intl.formatMessage({
              ...
              <div style={{ maxWidth: '332px', lineBreak: 'anywhere' }}>
                {curDrawerFundingVote?.rewardUrl}
              </div>
            </div>
          )}
          <div style={{ display: 'flex' }}>
          ...
          <div style={{ display: 'flex' }}>
            ...
            <div>
              {curDrawerFundingVote?.idolsToVote?.map((item, index) => (
                <Tag
                  key={index}
                  style={{
                    fontSize: '13px',
