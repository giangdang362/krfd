...
import { getRequestVoteCommunity } from '@/services/management/community';
import { deleteVote, postRejectOpenVote } from '@/services/management/vote';
...
import { Button, Drawer, Image, Modal, Popover, Spin, Table, Tag, Typography, message } from 'antd';
...
const DataRequestOpenVoteTable: FC<DataRequestOpenVoteTableProps> = ({
  ...
  const handleOpenChange = (x: boolean) => {
    setShowRejectModal(x);
  };
  ...
  const handleGetVoteData = async (isFilterChange?: boolean) => {
    ...
    const res = await getRequestVoteCommunity({
  ...
  }, [currentStatus, currentName, reload]);
    ...
    <div className="wrapp-table">
      ...
      >
        ...
        >
          <Image
            src={
              curRequestOpenVote?.bannerFileName?.length
                ? getUrlImage(curRequestOpenVote?.bannerFileName)
                : ''
            }
            style={{
              width: '440px',
        ...
        >
          ...
          <div style={{ display: 'flex' }}>
            ...
            >
              {curRequestOpenVote?.idols?.map((item) => (
                <Tag
                  key={item.id}
                  style={{
                    fontSize: '13px',
